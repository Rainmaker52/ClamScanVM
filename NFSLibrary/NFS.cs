...
namespace NFSLibrary
{
    ...
    public class NfsClient
    {
        ...
        public long Read(string sourceFileFullName, long offset, long totalLength, ref byte[] buffer)
        {
            ...
            {
                throw new ArgumentOutOfRangeException($"Buffer is too small to hold requested data. Buffer size {buffer.Length}, requested {totalLength}");
            }

            /* Prepare full Buffer to read */
            var tempBuffer = ArrayPool<byte>.Shared.Rent((int)totalLength); 
            var currentPosition = 0;

            try
            {
                var readCount = 0;
                var dataRemaining = totalLength;
                do
                {
                    var chunkBuffer = ArrayPool<byte>.Shared.Rent(blockSize);
                    var readInThisRequest = dataRemaining > blockSize ? blockSize : dataRemaining;
                    readCount = _nfsInterface.Read(sourceFileFullName, offset + currentPosition, (int) readInThisRequest, ref chunkBuffer);

                    DataEvent?.Invoke(this, new NfsEventArgs(readCount));

                    Buffer.BlockCopy(chunkBuffer, 0, buffer, currentPosition, readCount);

                    dataRemaining -= readCount;
                    currentPosition += readCount;
                    ArrayPool<byte>.Shared.Return(chunkBuffer, false);
                }
                while (readCount != 0 && dataRemaining > 0);
